...
import { Empleado, userResetPasswordReq, retriveEmpleadosReseteosReq, Domain, userUnlockReq, ApiServerInfo } from '../model/bussinessModel';
  ...
  oauthToken$(userName: string, password: string, domain: string): Observable<CurrentLogin> {
    ...
    return this.http.post<any>(AppConstants.AppOAuth_URL, bussinesData, AppConstants.httpClientOption_contenttype_json).pipe(
      map(res => {
        ...
        currentLogin.currentUser.CAIS = tokenInfo.CAIS.toLowerCase() as boolean;
