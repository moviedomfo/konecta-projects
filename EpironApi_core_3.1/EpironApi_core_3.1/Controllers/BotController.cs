...
namespace epironApi.webApi.Controllers
{
    ...
    public class BotController : ControllerBase
    {
        ...
        public async Task<IActionResult> authenticate(LoginRequest login)
        {
            ...
            using (var client = new HttpClient(proxy))
            {
                ...
                if (result.IsSuccessStatusCode)
                {
                    ...
                    //return Ok(jwt);
                    return Ok(new ApiOkResponse(jwt));
                }
                else
                {
                    return BadRequest(new ApiErrorResponse(HttpStatusCode.InternalServerError, result.Content.ToString())); ;
        ...
        /// <returns></returns>
        [HttpPost("[action]")]
        public  IActionResult responseBotComment(BotCommentModeratedReq req)
        {
            ...
            try
            {
                ...
                return Ok(new ApiOkResponse(new { caseId = req.CaseId, result= "OK"}));
